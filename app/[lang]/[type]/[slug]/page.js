...
export async function generateMetadata({ params }) {
  ...
  const turkishPath = consolidatedDestination(type, slug);
  const languages = { 'tr-TR': turkishPath, en: `/en/${type}/${slug}`, 'x-default': turkishPath };
...
export default async function LocalizedPage({ params }) {
  ...
  return <main lang={lang} dir={rtl ? 'rtl' : 'ltr'} className="min-h-screen bg-[#fcfaf7] text-[#222]">
    ...
    <article className="max-w-[900px] mx-auto px-6 py-16 md:py-24"><p className="font-serif text-2xl md:text-4xl leading-relaxed mb-16">{item.copy.intro(item.localizedTitle)}</p><section className="border-t border-[#ddd8cf] pt-10"><h2 className="font-serif text-3xl md:text-4xl mb-5">{item.copy.detailTitle}</h2><p className="font-serif text-lg leading-[1.9] text-[#555]">{item.copy.detail}</p></section><aside className="mt-14"><h2 className="font-serif text-2xl mb-5">{item.copy.related}</h2><nav className="flex flex-wrap gap-3"><Link href={consolidatedDestination(type, slug)} className="border px-4 py-3">Türkçe</Link>{locales.map((code) => <Link key={code} href={`/${code}/${type}/${slug}`} className="border px-4 py-3">{code.toUpperCase()}</Link>)}</nav></aside></article>
